...
// Types
import { SummaryContextType, UserNutritionNeeds } from '../../types/types';
type CalculatorInfoContentProps = {
	userNutritionNeeds: UserNutritionNeeds
}

export default function CalculatorInfoContent({ userNutritionNeeds } : CalculatorInfoContentProps) {
	// External logic/state
	const { setSummary } = useContext(SummaryContext) as SummaryContextType;
	const { isPending, postData } = useDataApi();
