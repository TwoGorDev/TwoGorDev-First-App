...
// Types
import { Portion, ProductsContextType, SummaryContextType } from '../../types/types';
type AddPortionModalProps = {
	title: string
	setIsAddPortionModalOpen: React.Dispatch<React.SetStateAction<boolean>>
	mealId: number
	mealPortions: Portion[]
}

export default function AddPortionModal({title, setIsAddPortionModalOpen, mealId, mealPortions} : AddPortionModalProps) {
	// External logic/state
	const { products, isPending, setEndpoint } = useContext(ProductsContext) as ProductsContextType;
	const { date, setSummary } = useContext(SummaryContext) as SummaryContextType;
	...
	const [addedPortions, setAddedPortions] = useState<Portion[]>([]);
	...
	const addPortion = (portion : Portion) => {
	...
	const removePortion = (portion: Portion) => {
